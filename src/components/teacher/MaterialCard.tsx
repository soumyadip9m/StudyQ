...
import { FileText, Download, Edit2, Trash2, Calendar, User, Tag, MoreVertical } from 'lucide-react';
import { DatabaseMaterial } from '../../types/database';
...
const MaterialCard: React.FC<MaterialCardProps> = ({ material, onUpdate, showActions = false }) => {
  const [showMenu, setShowMenu] = useState(false);

  const deleteMutation = useMutation(
    (materialId: number) => apiService.deleteMaterial(materialId),
    {
      onSuccess: () => {
        onUpdate();
      },
  ...
  const handleDelete = () => {
    if (window.confirm(`Are you sure you want to delete "${material.title}"?`)) {
      deleteMutation.mutate(material.material_id);
    }
    setShowMenu(false);
  };
      ...
      <div className="p-6">
                ...
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                  <div className="py-1">
                    ...
                    >
                      ...
                      {deleteMutation.loading ? 'Deleting...' : 'Delete Material'}
                    </button>
                  </div>
                </div>
              )}
        ...
        {material.tags && (
          <div className="flex flex-wrap gap-1 mb-4">
            ...
            {material.tags.split(',').length > 3 && (
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                +{material.tags.split(',').length - 3} more
              </span>
            )}
          </div>
        )}
        ...
        <div className="space-y-2 mb-4">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-gray-900">{material.academic_year}</span>
          </div>
          
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-500">File Size:</span>
            <span className="text-gray-900">{formatFileSize(material.file_size)}</span>
          </div>
